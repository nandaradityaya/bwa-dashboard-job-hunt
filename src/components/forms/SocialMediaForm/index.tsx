...
const SocialMediaForm: FC<SocialMediaFormProps> = ({ detail }) => {
  ...
  const onSubmit = async (val: z.infer<typeof socialMediaFormSchema>) => {
    try {
      // buat bodynya untuk di kirim ke API
      const body = {
        ...val,
        companyId: session?.user.id, // companyId ambil dari session
      };

      await fetch("/api/company/social-media", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body), // kirim bodynya
      });

      await router.refresh(); // refresh halamannya jika sudah di klik submit
