...
const PostJobPage: FC<PostJobPageProps> = ({}) => {
  ...
  const onSubmit = async (val: z.infer<typeof jobFormSchema>) => {
    // console.log(val);
    try {
      // buat variable body ambil dari prisma client
      const body: any = {
        applicants: 0,
        benefits: val.benefits,
        categoryId: val.categoryId,
        companyId: session?.user.id!!, // ambil session company yg login
        datePosted: moment().toDate(), // ambil date hari ini
        description: val.jobDescription,
        dueDate: moment().add(1, "M").toDate(), // ambil dari moment tanggal sekarang di tambah 1 bulan maka itu dueDatenya
        jobType: val.jobType,
        needs: 20, // set needsnya 20
      ...
      await fetch("/api/job", {
        ...
        body: JSON.stringify(body), // ubah bodynya jadi string
      });
  ...
  // gunakan useEffect agar CKEditor load dulu, karna klo CKEditor muncul sebelum halaman di load itu akan error. jadi halaman dulu baru CKEditor
  useEffect(() => {
              ...
              render={({ field }) => (
                <FormItem className="space-y-3">
                  <FormControl>
                    ...
                    >
                      {/* looping Job Types dari constant yg udah dibuat */}
